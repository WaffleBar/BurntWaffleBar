...
/// <summary>
/// Death Knight icons: preserve source color variety (steel/frost/bone/blood).
/// Same minimal pipeline as ProcessThePaladinIcons — no clock harmonization wash.
/// </summary>
public static class ProcessDeathKnightIcons
{
            ...
            using (var shadow = new Bitmap(w, h, PixelFormat.Format32bppArgb))
            {
                for (int y = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
                    {
                        ...
                        shadow.SetPixel(x, y, Color.FromArgb(sa, 8, 10, 18));
                    }
                }
    ...
    public static void ProcessAll(string inputDir, string outputDir)
    {
        ...
        {
            string input = Path.Combine(inputDir, "TheDeathKnight_" + name + ".png");
