...
public static class ProcessFrozenIcons
{
    ...
    static byte KeyAlpha(int r, int g, int b)
    {
        ...
        // Only remove near-pure-black background. Preserve dark blues and shadows in the art.
        if (r <= 10 && g <= 10 && b <= 10)
        {
            return 0;
        }

        // Feather anti-aliased neutral grays at the background edge only.
        if (max <= 24 && chroma <= 5)
        {
    ...
    public static void ProcessAll(string inputDir, string outputDir)
    {
        ...
        {
            string input = Path.Combine(inputDir, "FrozenWaffle_" + name + ".png");
